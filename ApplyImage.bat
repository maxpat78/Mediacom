...
REM Se non si indica il disco di installazione, si assume D:

REM Si assume l'installazione dell'immagine 2 del WIM (Windows 10 Home)
...
Dism /Apply-Image /ImageFile:%SRCDRIVE%\sources\install.wim /Index:2 /ApplyDir:W:\
...
REM REG ADD "HKLM\Offline\Microsoft\Windows NT\CurrentVersion" /ve RegisteredOrganization /t REG_SZ /f
...
rem Dism /Image:W:\ /ScratchDir:W:\Windows\TEMP /Add-Package /PackagePath:%SRCDRIVE%\kb3147458.msu
